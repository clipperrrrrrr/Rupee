...
#ifndef RUPEES_ZRSWALLET_H
#define RUPEES_ZRSWALLET_H
...
public:
    CzRSWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZRS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZRS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //RUPEES_ZRSWALLET_H
