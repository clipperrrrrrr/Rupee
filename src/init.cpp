...
void PrepareShutdown()
{
    ...
    RenameThread("digitalrupee-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "digitalrupee.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "digitalrupeed.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 55555, 11773));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, digitalrupee, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all DigitalRupee specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
...
#endif
//    strUsage += "  -anonymizedigitalrupeeamount=<n>     " + strprintf(_("Keep N DRS anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The DigitalRupee Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("digitalrupee-loadblk");
...
/** Sanity checks
 *  Ensure that DigitalRupee is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. DigitalRupee Core is shutting down."));
...
#endif
    // Make sure only a single DigitalRupee process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. DigitalRupee Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("DigitalRupee version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //DigitalRupee specific: zerocoin and spork DB's
                ...
                // DigitalRupee: load previous sessions sporks if we have them.
                ...
                // DigitalRupee: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of DigitalRupee Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart DigitalRupee Core to complete") << "\n";
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeDigitalRupeeAmount = 2;
...
//    }
//
//    nAnonymizeDigitalRupeeAmount = GetArg("-anonymizedigitalrupeeamount", 0);
//    if (nAnonymizeDigitalRupeeAmount > 999999) nAnonymizeDigitalRupeeAmount = 999999;
//    if (nAnonymizeDigitalRupeeAmount < 2) nAnonymizeDigitalRupeeAmount = 2;
    ...
    LogPrintf("Anonymize DigitalRupee Amount %d\n", nAnonymizeDigitalRupeeAmount);
