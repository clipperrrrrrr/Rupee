...
public:
    ...
    bool WriteZRSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZRSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZRSSeed_deprecated(uint256& seed);
    bool EraseZRSSeed();
    bool EraseZRSSeed_deprecated();

    bool WriteZRSCount(const uint32_t& nCount);
    bool ReadZRSCount(uint32_t& nCount);
