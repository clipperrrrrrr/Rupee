...
CzRSWallet::CzRSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zrs seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZRSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZRSSeed_deprecated()) {
                    LogPrintf("%s: Updated zRS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zrs seed\n", __func__);
                }
            }
        }
    }
...
bool CzRSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZRSCount(nCountLastUsed);
    else if (!walletdb.ReadZRSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzRSWallet::Lock()
{
    seedMaster = 0;
}

void CzRSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzRSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZRS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzRSWallet::LoadMintPoolFromDB()
{
...
void CzRSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzRSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzRSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zrsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzRSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZRS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zrsTracker which also adds to database
    pwalletMain->zrsTracker->Add(dMint, true);
...
void CzRSWallet::SeedToZRS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzRSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzRSWallet::GenerateDeterministicZRS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzRSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzRSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
