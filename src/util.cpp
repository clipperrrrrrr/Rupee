...
int nAnonymizeDigitalRupeeAmount = 1000;
...
#else
    const char* pszModule = "digitalrupee";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "digitalrupee.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty digitalrupee.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override digitalrupee.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "digitalrupeed.pid"));
