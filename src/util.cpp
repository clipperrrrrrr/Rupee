...
int nAnonymizeRupeesAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "rupees" is a composite category enabling all Rupees-related debug output
            if (ptrCategory->count(string("rupees"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "rupees.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty rupees.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override rupees.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "rupeesd.pid"));
