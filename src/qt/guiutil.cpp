...
#define URI_SCHEME "digitalrupee"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a DigitalRupee address (e.g. %1)").arg("PCYiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
{
    // return if URI is not valid or is no DigitalRupee: URI
...
{
    // Convert digitalrupee:// to digitalrupee:
    //
    //    Cannot handle this later, because digitalrupee:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open digitalrupee.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DigitalRupee.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for DigitalRupee.lnk
...
{
    return GetAutostartDir() / "digitalrupee.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a digitalrupee.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=DigitalRupee\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the digitalrupee app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add digitalrupee app to startup item list
