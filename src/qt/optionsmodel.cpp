...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::RS);
    ...
    if (!settings.contains("nAnonymizeRupeesAmount"))
        settings.setValue("nAnonymizeRupeesAmount", 1000);

    nAnonymizeRupeesAmount = settings.value("nAnonymizeRupeesAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeRupeesAmount"))
        SoftSetArg("-anonymizerupeesamount", settings.value("nAnonymizeRupeesAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in rupees.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeRupeesAmount:
            return QVariant(nAnonymizeRupeesAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeRupeesAmount:
            nAnonymizeRupeesAmount = value.toInt();
            settings.setValue("nAnonymizeRupeesAmount", nAnonymizeRupeesAmount);
            emit anonymizeRupeesAmountChanged(nAnonymizeRupeesAmount);
            break;
        case CoinControlFeatures:
