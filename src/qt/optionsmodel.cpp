...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeDigitalRupeeAmount"))
        settings.setValue("nAnonymizeDigitalRupeeAmount", 1000);

    nAnonymizeDigitalRupeeAmount = settings.value("nAnonymizeDigitalRupeeAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDigitalRupeeAmount"))
        SoftSetArg("-anonymizedigitalrupeeamount", settings.value("nAnonymizeDigitalRupeeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in digitalrupee.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDigitalRupeeAmount:
            return QVariant(nAnonymizeDigitalRupeeAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDigitalRupeeAmount:
            nAnonymizeDigitalRupeeAmount = value.toInt();
            settings.setValue("nAnonymizeDigitalRupeeAmount", nAnonymizeDigitalRupeeAmount);
            emit anonymizeDigitalRupeeAmountChanged(nAnonymizeDigitalRupeeAmount);
            break;
        case CoinControlFeatures:
