                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zRSpayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);
    

    // "Spending 999999 zRS ought to be enough for anybody." - Bill Gates, 2017
    ui->zRSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzRSSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zRSpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzRS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zRS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zRS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zRS in ") +
...
void PrivacyDialog::on_pushButtonSpendzRS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zRS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zRS
        sendzRS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zRS
    sendzRS();
}

void PrivacyDialog::on_pushButtonZRsControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZRsControlDialog* zRsControl = new ZRsControlDialog(this);
    zRsControl->setModel(walletModel);
    zRsControl->exec();
}

void PrivacyDialog::setZRsControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzRSSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzRS()
{
    ...
    double dAmount = ui->zRSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zRS is requested
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zRS</b>";
    ...
    // use mints from zRS selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZRsControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZRsControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zRS require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zRS"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZRS_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zRS require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zRS"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zRS was spent successfully update the addressbook with the label
    ...
    // Clear zrs selector in case it was used
    ZRsControlDialog::setSelectedMints.clear();
    ui->labelzRSSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zRS Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zrsTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zRS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zRS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zRS "));
    ui->labelzRSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzRSSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzRS->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zRS
        ui->pushButtonMintzRS->setEnabled(false);
        ui->pushButtonMintzRS->setToolTip(tr("zRS is currently disabled due to maintenance."));

        // Spend zRS
        ui->pushButtonSpendzRS->setEnabled(false);
        ui->pushButtonSpendzRS->setToolTip(tr("zRS is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zRS
        ui->pushButtonMintzRS->setEnabled(true);
        ui->pushButtonMintzRS->setToolTip(tr("PrivacyDialog", "Enter an amount of DRS to convert to zRS", 0));

        // Spend zRS
        ui->pushButtonSpendzRS->setEnabled(true);
        ui->pushButtonSpendzRS->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}