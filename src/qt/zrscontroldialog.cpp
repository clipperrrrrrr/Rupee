...
std::set<std::string> ZRsControlDialog::setSelectedMints;
std::set<CMintMeta> ZRsControlDialog::setMints;

ZRsControlDialog::ZRsControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZRsControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZRsControlDialog::~ZRsControlDialog()
{
    delete ui;
}

void ZRsControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZRsControlDialog::updateList()
{
...
void ZRsControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZRsControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZRs_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZRsControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZRsControlDialog::GetSelectedMints()
{
...
void ZRsControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
